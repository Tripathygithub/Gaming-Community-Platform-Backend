...
const tournaments = [
  { id: 1, gameId: 1, name: "Valorant Championship", date: "2022-12-01", prizePool: 5000 },
  { id: 2, gameId: 2, name: "FIFA World Cup", date: "2022-11-15", prizePool: 3000 },
  { id: 3, gameId: 3, name: "Among Us Showdown", date: "2022-10-20", prizePool: 2000 },
];
...
db.serialize(() => {
  ...
  const stmt = db.prepare("INSERT INTO games (title, genre, platform, rating) VALUES (?, ?, ?, ?)");
  const stmt2 = db.prepare("INSERT INTO players (name, username, platform, rating) VALUES (?, ?, ?, ?)");
  const stmt3 = db.prepare("INSERT INTO tournaments (gameId, name, date, prizePool) VALUES (?, ?, ?, ?)");
  ...
  for (let tournament of tournaments) {
    stmt3.run(tournament.gameId, tournament.name, tournament.date, tournament.prizePool);
  }
